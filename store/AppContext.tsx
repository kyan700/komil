...
import { deleteRecordingFile } from "@/lib/recordings";
...
interface AppState {
  ...
  // Recordings
  addRecording: (input: Omit<Recording, "id" | "createdAt">) => Recording;
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const [t, sub, sch, inb, foc, lec, rec, str, name, uni, maj, lvl, ob] = await Promise.all([
        ...
        loadJson<Recording[]>(STORAGE_KEYS.recordings, []),
  ...
  useEffect(() => { if (ready) saveJson(STORAGE_KEYS.recordings, recordings); }, [recordings, ready]);
  ...
  const addRecording: AppState["addRecording"] = useCallback((input) => {
    const rec: Recording = {
      ...input,
      id: generateId(),
      createdAt: new Date().toISOString(),
  ...
  const deleteRecording = useCallback((id: string) => {
    setRecordings((prev) => {
      ...
      if (target?.uri) deleteRecordingFile(target.uri);
  ...
  const value = useMemo<AppState>(
    ...
    [
      ...
      tasks, subjects, schedule, inbox, focusSessions, lectures, recordings, streak,
